<template>
  <v-stepper v-model="step">
    <v-stepper-header>
      ...
      <v-divider/>
      <v-stepper-step step="4">SKU特有属性</v-stepper-step>
    </v-stepper-header>
    <v-stepper-items>
      <!--1、基本信息-->
...
export default {
  ...
  methods: {
    submit() {
      ...
      // 处理规格参数
      const specs = {};
      this.specs.forEach(({ id,v }) => {//处理通用规格参数
        specs[id] = v;//1:LG   2:Mate 20
      });
      // 处理特有规格参数模板
      const specTemplate = {};
      this.specialSpecs.forEach(({ id, options }) => {
        specTemplate[id] = options;//4:["红色","蓝色"] 循环取出特有的规格参数
      });
      // 处理sku
      ...
      goodsParams.spuDetail.genericSpec = JSON.stringify(specs);//把{1:"华为"}这样的通用规格对象，转为字符串json结构
      goodsParams.spuDetail.specialSpec = JSON.stringify(specTemplate);//把{4:["红色","蓝色"]}这样的特有规格可选参数列表对象，转为字符串json结构
      ...
      handler(val) {
        if (!this.isEdit) {//新增
          ...
          this.goods.skus.forEach(s => {
            skuMap.set(s.indexes, s);//{0_0_0:skuObj}
          });
        ...
        if (val && val.length > 0) {
          ...
          this.$http
            ...
            .then(({ data }) => {
              ...
              if (this.isEdit){//修改
                specs = JSON.parse(this.goods.spuDetail.genericSpec);//取得通用规格参数的值
                template = JSON.parse(this.goods.spuDetail.specialSpec);//取得特有规格参数的值
              }
              ...
              data.forEach(({id, name,generic, numeric, unit }) => {
                if(generic){//判断是否通用属性
                  ...
                  if(this.isEdit){//同时要取得对应的规格属性的值，并且封装到一个叫v的属性的中
                    o.v = specs[id];
                  }
                  arr1.push(o)
                }else{//特有规格属性
